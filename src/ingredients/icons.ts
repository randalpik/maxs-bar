...
export function icon(shape: string, color: string): string {
  ...
  switch (shape) {
    ...
    // Faux-3D cheese wedge: top face + front depth band (shaded), holes on the top.
    case 'cheese': inner = `<path d="M6 17 L25 17 L25 20.5 Q25 21.5 24 21.5 L7 21.5 Q6 21.5 6 20.5 Z" fill="${color}" ${w}/><path d="M6 17 L25 17 L25 20.5 Q25 21.5 24 21.5 L7 21.5 Q6 21.5 6 20.5 Z" fill="${S}" opacity=".18"/><path d="M6 17 L22 7.5 Q24 6.5 25 9 L25 17 Z" fill="${color}" ${w}/><circle cx="13" cy="13.5" r="1.4" fill="none" stroke="${S}" stroke-width="1" opacity=".55"/><circle cx="19" cy="12" r="1" fill="none" stroke="${S}" stroke-width="1" opacity=".55"/>`; break;
